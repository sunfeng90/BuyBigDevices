...
export default {
  ...
  data() {
    ...
    return {
      ...
      columns: [// 表头数据
        ...
        { label: "估算价格（万元）", prop: "price" },
        { label: "主要功能和用途", prop: "functions" },
        { label: "主要性能参数", prop: "parameters" },
        { label: "预期效益", prop: "benefit" },
        { label: "主要使用学科", prop: "subject" },
        { label: "可共享的其他学科", prop: "sharesubject" },
        { label: "建议人", prop: "person" },
        { label: "联系电话", prop: "tel" },
        { label: "所在单位(具体到研究室)", prop: "depart" },
      ],
      pageObj: {//分页数据
        sizes: [10, 20, 30, 40],
        size: 10,
  ...
  methods: {
    ...
    getDeviceList(val) {
      ...
      self.$api.tableApi.deviceList(obj).then((res) => {
        self.tableData = [];
        if (res) {
